...
namespace test_int {
    typedef VectorClass<int>    IntVec;
    void    print_elements(IntVec::iterator from, IntVec::iterator to) {
        std::cout << "[";
        for (IntVec::iterator it = from; it < to; ++it) {
            if (it != from) {
                std::cout << ", ";
            }
            std::cout << *it;
    ...
    void    swap() {
        ...
        vj.push_back(3);
        vj.push_back(4);
        IntVec::iterator ii = vi.begin();
        IntVec::iterator ij = vj.begin();
